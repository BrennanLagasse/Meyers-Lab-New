...
# Takes original definition for conversion and modifies
# New parameters are:
# baselinePeriodLength (integer for baseline period length)
# stimulusPeriodLength (ineger for stimulus period length)
# dataPerPeriod (number of observations during that period)

# This solution may not generalize well, but it works for this data

convert_matlab_raster_data_modified <- function(matlab_raster_dir_name,
                                       baselinePeriodLength,
                                       stimulusPeriodLength,
                                       dataPerPeriod,
                                       start_ind = NULL,
                                       end_ind = NULL,
  ...
  for (iSite in seq_along(matlab_file_names)) {
    ...
    # Change the times to reflect how there is not always a measure
    for (i in 1:dataPerPeriod) {
      start = floor(-baselinePeriodLength + ((baselinePeriodLength + stimulusPeriodLength)/dataPerPeriod * (i - 1)))
      end = floor(-baselinePeriodLength + ((baselinePeriodLength + stimulusPeriodLength)/dataPerPeriod * i))
      
      new_times[i] = paste(start, "_", end, sep = "")
    }
    
    
    names(raster_data) <- paste0("time.", new_times)
...
convert_one_session <- function(session_name, start, end, measuresPerPeriod) {
  ...
  convert_matlab_raster_data_modified(matlab_raster_dir_name, start, end, measuresPerPeriod, r_raster_dir_name = r_raster_dir_name)
